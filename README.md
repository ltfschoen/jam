...
* [Graypaper Typography/Notation](#graypaper-notation)
* [Glossary](#glossary)
* [Tools](#tools)
...
* Userland level (i.e. Parachains)
  ...
  * **CoreChains Service**
    * First service deployed in JAM will likely be called CoreChains Service (or Parachains Service) and support allowing existing Polkadot-2-style parachains to be migrated to and executed on JAM.
    ...
    * Scaling Out (Horizontal) (e.g. Ethereum and Polkadot)
      * Reducing the number of machines that have to executing all the work
        ...
        * In blockchains, the "computation machine" is the entire validator set of a network so we want to reduce the workload on the entire validator set as a whole to scale out the system, which is achieved by either:
          ...
          * SNARK-based rollups
      * Benefit of only requiring capacity for more validators to increase the throughput rather than:
        * Limited by validator throughput being the minimum service specification required of a validator multiplied by the minimum interconnect required of the validator set, and beyond that unable to scale up more until more performance hardware released.
  * Transactions
    * JAM is transactionless, everything is triggered by the servers used by users

  * PVM (PolkaVM)
    * Efficient gas metering
    * Ability to pause and resume execution, which is important for CorePlay for implementing new things with
    * PVM can invoke a parachain that is a PVM child blob and so forth
...
* Hardware level (i.e. Cores)
    ...
    * Distribute DA floats/hangs there without having to go to a distributed storage solution nor go on On-Chain, it can just be there ready for usage for up to 28 days
		...
		* Cores (uses PVM) each are In-Core and `refine` (aka Join) Work Items from each of their Work Packages
...
* JAM
  * Goals
    ...
    * Accessibility - no negligible barriers to innovation, easy, fast, cheap and permissionless (e.g. no slot auctions like in Polkadot where need own chain with collators, does not require much financial or community support to deploy to JAM).
  ...
  * Features
    ...
    * Dualism that is secure and scalable 
      * In-core consensus computation (similar to optimistic roll-up in Ethereum, it is performed off-chain with correctness guaranteed through game theoretic mechanisms that are rooted on-chain)
      * On-chain consensus computation (e.g. determining spent vs unspent outputs)

    * JAM Service - see Glossary definition
    ...
    * Size-synchrony Antagonism 
      ...
      * Trade-off in systems between the "size" and the "synchrony" (i.e. internal "coherency") between the parts of the system
        * High "size" by Scaling-up systems causes:
          * Low "synchronous" (changes in one part take longer to affect a distant part)
          * High "asynchronous"
          * Low "coherency"
        * Solution:
          * Design system that works around the trade-off limitation
          * JAM presents a new compromise for blockchain systems that handles the trade-off by having the right amount of "size" and "synchrony"
        * Example:
          * Ethereum 1 was Low "size" and High "synchrony" (i.e. smart contracts able to immediately call any other smart contract in the system, state is homogenous in terms of accessibility)
          * Polkadot has High "size" (many blockchains each with their own state) and Low "synchrony" (Low "coherency")
            * Bridges are attempts to solve Low "synchrony"/"coherency" but they are insecure, high latency
          * Machine architecture registers (few of them) and L1 cache have Low "size" and High "synchrony"
            * L2, L3 cache, RAM, secondary storage (hard drives), virtual memory, remote memory, are attempts at Higher "size" levels to grow memory, but causing Lower "synchrony" (i.e. if CPU has one thing to do it has to wait for the response from memory, instead of having its own memory)
      * Proof of the "Size-synchrony Antagonism" principle is included in the Graypaper
      * Example:
...
* Accords
  * Allows Services to have shared agreement between each other 
  * Allow XCMP to be executed much easier
* Blockchain
...
* CoreJam
  * Original JAM RFC on Github that was an idea of how to change the Polkadot protocol. JAM improved the idea and transformed into a coherent protocol
* Coretime
  * Polkadot's equivalent of blockspace
  * Formally what you are able to do in a core over a period of time that is quantised over a period of blocks.
  * Metric of Web3 compute power (that is resilient, secure, ubiquitous)
  * Similar to Ethereum's EVM gas
...
* Ethereum 2
  * Dank Sharding Availability System
    ...
    * Use of ZK SNARKs in PLONK as part of the Dank Sharding availability system uses a form of erasure coding centered around polynomial commitments over a large prime field in order to allow SNARKs to get acceptably performant access to subsections of data. Compared
    ...
    * ZK SNARKs
      ...
      * Verifiable nature of ZK SNARKs avoids requiring benefits of staked decentralization 
  ...
  * Costly
    * ZK Roll-ups are extremely costly, millions of times more costly than a crypto-economic system that Polkadot uses
  * Centralizing
    * Problem: ZK provides proof of correctness, and it matters if only one entity does the ZK because then you have an ecosystem where only the cheapest actor is "active", and that single "active" actor can choose when they want to go offline to disrupt and manipulate financial markets, because if they go offline then it is costly and takes time to spin up a replacement actor.
  * Patchy
    * Heterogenous, and High incoherent (Asynchronous) approach with different scaling approaches and standards used by the different Ethereum L2 providers. It is not an effective way to scale-up the "size" an application using multiple scaling providers.
* Fragmented Meta-Networks (for computation scalability)
  * Fragmented approach / Bridged Chains (e.g. Cosmos, Avalanche)
    * Bridged chains are insecure, operating under different economic safeguards
      * Communication betwen bridged chains may result in the chain with the lower economic safeguards may have a problematic effect on the chain with the higher economic safeguards.
        * If you make them use the same economic safeguards (e.g. using same validator set) may be costly, and bridge chains are not doing this.
          * Note: Omniledger uses bridges in its design and has a sensible way of using the same validator set, but suffers from "Persistent State Fragmentation" similar to Polkadot.
...
* Graypaper
  * Unambiguous formal specification (using the Graypaper it should be possible to implement the JAM protocol and it should be compatible with other implementations of the protocol) that is fully detailed, similar to the Ethereum Yellowpaper. It is not a vision document like the Polkadot paper.
* Heterogeneous
...
* High-Performance Synchronous
  * Less Resilience risk (e.g. Scaling-Up require nodes to have huge amount of RAM, scale is limited by silicon chip hardware manufacturers latest release, ignore on-chain storage and just store off-chain on Google Cloud Bigtables)
  * Ignores Scaling-Out
* In-Core Execution
  * Operations inside a core. Abundance, scalable, as secure as on-chain execution through crypto-economics and ELVES.
* JAM
  * Driving Factors of continued innovation are Web3 maxims including:
      * Resilience - Build a Web3 protocol
      * Generality - Sufficient to enable numerous applications
      * Performance - Measure of:
        * How much computation possible in a period of time
        * Lowest possible cost of computation in a period of time
      * Coherency
        * How easy it is to interoperate between one part of the system and other parts of the system, and whether the ease of doing so is:
          * Same (homogenous) level of ease
          * Varied (structurally different) level of ease depending on which part of the system you are interoperating with
        * Important when compare with different directions taken for Scaling
      * Accessibility - How easy to use the system and extent of availability everywhere
    * Improve upon existing Web3 protocols
  ...
  * Allows arbitrary data to be read-from and written-into the Polkadot Data Availability (DA) layer and must be propogated across all validators.
  ...
  * Work Package
    * Group of Work Items that belong to a Service
  ...
  * JAM brings the ability to not have to have "Persistent State Partitioning" like in Polkadot
  * JAM Analogy
...
* JAM Services
  ...
  * Described by model of computation with Entry Points:
    * Collect
    * Refine `fn refine()`
      * In-Core aspects of what the Service does
      * gas available is abundant
      * each core may access the Data Availability layer in parallel with ~28 day persistence
      * no direct access to JAM state, only after accumulate by all validators
    ...
    * Join (from JAM)
    * Accumulate `fn accumulate()`
      * On-Chain aspects of what the Service does, sequential like a traditional blockchain
      * direct access to whole JAM state (stateful)
      * less gas available
      * fully "Synchronous"
      * `transfer` between cores
    * `on_transfer` - Services send messages to each other post-accumulate
    * e.g.
  ...
  * CoreChains Service
    * Polkadot-compatible and where model largely based on architecture of Polkadot
      * Provides an abstraction closer to the actual computation model generated by the validator nodes its economy incentives that was not offered previously with Polkadot
  * Service is used to allow Jam to be programmable using using PVM bytecode
  * Services that are invoked are called a Work Item
* JAM State
  * Guarantee by each core to check whether core and validators have executed a Work Package and are satisfied
  * Deal with Services (page 8 of JAM paper), is analogous to smart contract accounts in the Ethereum Yellow Paper
* On-Chain Execution
...
* Persistent State Partitioning
    * Polkadot oversees parachains that each have a State Machine with some State (represented as a Merkle Root), where that State is persistently split between each parachain (i.e. Polkadot does not support moving a smart contract from the Moonbeam parachain to the Astar parachain), which causes Polkadot when viewed as a single overarching entity (it operates under a single consensus umbrella, different to the Cosmos system) and its Inter-Parachain communication to have a Size-synchrony Antagonism that is High "size" (large State) but is High "asynchronous" (e.g. if you send an XCM message between parachains it takes a certain amount of blocks to arrive), whereas in Ethereum 1 with its Intra-Parachain communication is Low "size" and High "synchronous" (single chain, single state, where "synchronous" composability is easy).
      * JAM will have a different Size-synchrony Antagonism, with a reduced "size" of Inter-Parachain communication, and reduced "synchrony" of Intra-Parachain communication
...
* Polkadot
  ...
  * It is a network, a technology stack, an economy, and a community
...
### Generate Custom Notation Summary of Graypaper in LaTex
...
### Ad-Hoc Notation Notes

> To be incorporated into Custom Notation Summary

* 3.2 Functions and Operators
  - Precedes relation - function defined in part on the basis of another function
  - Substitute-if-nothing function - ignore all the arguments that are the empty set (null value) and just take the first one that is not. If all of them are the null value then return the null value.

* 3.3 Sets
  - Set disjointness
  - Question mark operator - turns the Set into the Set or the empty set (null, non element)
  - Upside-down delta - represents error

* 3.4 Numbers
  - Set of natural numbers
  - Subscript used to limit the maximum value in the Set of natural numbers (e.g. N_2^32, means a 32-bit unsigned quantity)
  - Division with remainder (i.e. `1 R 3`)

* 3.5 Dictionaries
  - Dictionaries are used in the Graypaper when we expect the elements to be enumerable and when able to add/remove elements as needed, whereas Functions are used strictly for "operations"
  - May use different Sets for keys and values (domains and co-domains)
  - Notation (i.e. similar to `K(d)` and `V(d)`) allowing us to take the keys of a dictionary (active domain) or the values 

* 3.6 Tuples
  - Cross-operator i.e. `NxN` takes two sets and makes an element drawn from one Set and an element drawn from another Set used as a Pair. Instead in the Graypaper the equivalent parenthesised tuple notation is used i.e. `t = (3,5)` or "named tuple" notation where you can specify a name for each element of the tuple, and then subscript by that name, where each element may be a natural number i.e. `t = (a: 3, b: 5) ... t_a = 3, t_b = 5`.
...
* Blogs
  * https://github.com/PhucLam202/Polkadot-JAM-Chain
* Graypaper Lectures
  * [X] 1.1-Nomenclature https://graypaper.com/lectures/?section=1.1-Nomenclature
  * [X] 1.2-Driving-Factors https://graypaper.com/lectures/?section=1.2-Driving-Factors
  * [X] 1.3-Scaling-Under-Size-Synchrony-Antagonism https://graypaper.com/lectures/?section=1.3-Scaling-Under-Size-Synchrony-Antagonism
  * [X] 2.1-Polkadot https://graypaper.com/lectures/?section=2.1-Polkadot
  * [X] 2.2-Ethereum https://graypaper.com/lectures/?section=2.2-Ethereum
  * [X] 2.4-High-Performance-Fully-Synchronous-Networks https://graypaper.com/lectures/?section=2.4-High-Performance-Fully-Synchronous-Networks
  * [X] 3.1-Typography https://graypaper.com/lectures/?section=3.1-Typography
  * [X] 3.2-Functions-and-Operators https://graypaper.com/lectures/?section=3.2-Functions-and-Operators
